...
void run_user_interface(int *argc, char ***argv) {
    ...
    int new_argc = 2;
    if (strcmp(algorithm, "iter") != 0) new_argc += 2;
    if (strcmp(format, "dec") != 0) new_argc += 2;
    if (show_time) new_argc += 1;
    if (raw_output) new_argc += 1;
    if (verbose) new_argc += 1;
    if (output_file) new_argc += 2;
    ...
    if (show_time) new_argv[arg_index++] = strdup("-t");
    if (raw_output) new_argv[arg_index++] = strdup("-r");
    if (verbose) new_argv[arg_index++] = strdup("-v");
